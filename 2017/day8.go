...
func (i inst) apply(regs map[string]int) {
	var cond bool
	switch i.condOp {
	case ">":
		cond = regs[i.condReg] > i.condVal
	case "<":
		cond = regs[i.condReg] < i.condVal
	case ">=":
		cond = regs[i.condReg] >= i.condVal
...
func parse(s string) []inst {
	...
	for _, line := range utils.Lines(s) {
		...
		in.reg, in.op, in.condReg, in.condOp = fs[0], fs[1], fs[4], fs[5]
		in.delta, in.condVal = utils.Atoi(fs[2]), utils.Atoi(fs[6])
