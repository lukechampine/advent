...
type op1 struct {
	x1, y1 int
	x2, y2 int
	action func(bool) bool
}

func (o op1) apply(lights *[1000][1000]bool) {
	for x := o.x1; x <= o.x2; x++ {
		for y := o.y1; y <= o.y2; y++ {
			lights[x][y] = o.action(lights[x][y])
		}
	}
}

func parseOp1(s string) op1 {
	var o op1
	switch {
...
func (o op2) apply(lights *[1000][1000]int) {
	for x := o.x1; x <= o.x2; x++ {
		for y := o.y1; y <= o.y2; y++ {
			lights[x][y] = o.action(lights[x][y])
		}
