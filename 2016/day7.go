...
const input = `wysextplwqpvipxdv[srzvtwbfzqtspxnethm]syqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt
...
func (addr ip) ABAs() []string {
	var abas []string
	for _, o := range addr.outside {
		for i := 2; i < len(o); i++ {
			if o[i] == o[i-2] && o[i] != o[i-1] {
				abas = append(abas, o[i-2:i+1])
			}
		}
	}
	return abas
...
func (addr ip) hasBAB(aba string) bool {
	bab := string([]byte{aba[1], aba[0], aba[1]})
	for _, i := range addr.inside {
		if strings.Contains(i, bab) {
			return true
		}
	}
	return false
...
func (addr ip) supportsSSL() bool {
	for _, aba := range addr.ABAs() {
		if addr.hasBAB(aba) {
			return true
		}
	}
...
func filterIPs(addrs []ip, pred func(ip) bool) []ip {
	var filtered []ip
...
func parse(str string) []ip {
	var addrs []ip
	for _, l := range utils.Lines(str) {
		var addr ip
		for _, open := range strings.Split(l, "[") {
			strs := strings.Split(open, "]")
			if len(strs) == 1 {
				addr.outside = append(addr.outside, strs[0])
			} else {
				addr.inside = append(addr.inside, strs[0])
				addr.outside = append(addr.outside, strs[1])
			}
		}
...
func main() {
	// part 1
	ips := parse(input)
	println(len(filterIPs(ips, ip.supportsTLS)))

	// part 2
	println(len(filterIPs(ips, ip.supportsSSL)))
}
