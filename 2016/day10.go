...
const input = `bot 37 gives low to bot 114 and high to bot 150
...
type bot struct {
	id                int
	value             int
	givelow, givehigh *bot
}

var specialBotID int
...
func (b *bot) give(value int) {
	...
	low, high := utils.Min(value, b.value), utils.Max(value, b.value)
	if low == 17 && high == 61 {
		specialBotID = b.id
	}
	if b.givelow != nil {
	...
	if b.givehigh != nil {
		b.givehigh.give(high)
	}
}

var outs = [3]bot{}

func parse(str string) ([]bot, int, int) {
	bots := make([]bot, len(utils.Lines(str)))
	var startBot, startVal int
	for _, line := range utils.Lines(str) {
		if strings.HasPrefix(line, "value") {
			var val, botid int
			utils.Sscanf(line, "value %d goes to bot %d", &val, &botid)
			b := bots[botid]
			if b.value != 0 {
				startBot = botid
				startVal = val
			...
			var botid int
			utils.Sscanf(line, "bot %d", &botid)
			b := bots[botid]
			b.id = botid
			i := strings.Index(line, "low to ") + len("low to ")
			rest := line[i:]
			var out int
			if strings.HasPrefix(rest, "output") {
				utils.Sscanf(rest, "output %d", &out)
				if out < 3 {
					b.givelow = &outs[out]
				}
			} else {
				utils.Sscanf(rest, "bot %d", &out)
				b.givelow = &bots[out]
			}
			i = strings.Index(line, "high to ") + len("high to ")
			rest = line[i:]
			if strings.HasPrefix(rest, "output") {
				utils.Sscanf(rest, "output %d", &out)
				if out < 3 {
	...
	return bots, startBot, startVal
}

func main() {
	// part 1
	bots, startBot, startVal := parse(input)
	bots[startBot].give(startVal)
	println(specialBotID)

	// part 2
	println(outs[0].value * outs[1].value * outs[2].value)
}
