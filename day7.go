...
func lookup(a string, m map[string]uint16) uint16 {
...
func calculate(actions map[string]action, wire string, memo map[string]uint16) uint16 {
	action := actions[wire]
	ins := make([]uint16, len(action.inputs))
	for i, input := range action.inputs {
		if num, err := strconv.Atoi(input); err == nil {
			ins[i] = uint16(num)
		} else if num, ok := memo[input]; ok {
			ins[i] = num
		} else {
			ins[i] = calculate(actions, input, memo)
			memo[input] = ins[i]
		}
	}
	return action.fn(ins...)
}
...
func main() {
	...
	println(calculate(actions, "a", memo))
}
